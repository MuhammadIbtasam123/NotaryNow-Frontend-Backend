...
function EditPdfView() {
  ...
  const fetchPdf = async () => {
    ...
    const notaryName = docid.split("+")[2];
  ...
  const addStamp = async () => {
    ...
    const notaryId = docid.split("+")[1];

    try {
      const response = await axios.get(
        `http://localhost:8080/api/get-stamp/${notaryId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
      ...
      console.log("Stamp data:", response.data);
  ...
  const saveAndReleaseDocument = async () => {
    ...
    try {
      ...
      for (const obj of canvasObjects) {
        if (obj.type === "image") {
          const imgData = await axios.get(obj._element.src, {
            responseType: "arraybuffer",
          });
          const embeddedImage = await pdfDoc.embedJpg(imgData.data);
          const { width, height, left, top } = obj;
          const scaledWidth = width * obj.scaleX;
          const scaledHeight = height * obj.scaleY;
          const x = left;
          const y = pageHeight - (top + scaledHeight);
